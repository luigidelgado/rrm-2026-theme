import { validateBikeForm } from "./scripts-c/profile/validation-forms.js";
jQuery(document).ready(function () {
  ...
  jQuery("#search_btn_mobile").on("click", function (e) {
    ...
    jQuery(".notifications_mobile").removeClass("show_notifications_mobile");
  ...
  jQuery('form[name="contact-me"]').on("submit", function (e) {
    ...
    jQuery
      .post(pm_ajax_object.ajax_url, data, function (response) {
        jQuery(".loader").hide();
        if (response === true || response === "true") {
          jQuery(".respuesta-contacto").text("Hemos recibido tu comentario.");
...
jQuery(window).on("hashchange", (event) => {
  const jQueryactiveAccordion = jQuery(window.location.hash);
  const jQueryparentAccordion = jQueryactiveAccordion.parents(".h-accordion");
