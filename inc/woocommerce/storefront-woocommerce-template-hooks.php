...
//remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_title',999);
...
add_action( 'woocommerce_rrm_before_main_content', 'rrm_content_wrapper_before', 10 );
add_action( 'woocommerce_rrm_after_main_content', 'rrm_content_wrapper_after', 11 );
add_action( 'woocommerce_rrm_before_sidebar', 'storefront_sidebar_product_search', 10 );

/* Loop producto */
remove_action('woocommerce_after_shop_loop_item', 'woocommerce_template_loop_product_link_close',10);
add_action('woocommerce_before_shop_loop_item_title','woocommerce_template_loop_product_link_close',11);

add_action('woocommerce_before_shop_loop_item_title','woocommerce_template_loop_product_wrapper_information_open',12);
add_action('woocommerce_after_shop_loop_item','woocommerce_template_loop_product_wrapper_information_close', 10);


/***  Store Layout ***/

/* Add coupon inside lateral content in checkout */
// remove_action('woocommerce_before_checkout_form', 'woocommerce_checkout_coupon_form', 10);
// add_action( 'woocommerce_checkout_order_review', 'woocommerce_checkout_coupon_form', 19 );

/* Change order of appareance of description */
add_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_excerpt',9);
remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_excerpt',20);

/* Add Offer to image container */
remove_action('woocommerce_before_single_product_summary' , 'woocommerce_show_product_sale_flash');
add_action( 'woocommerce_product_single_tag', 'woocommerce_show_product_sale_flash');

add_action('woocommerce_single_product_summary', 'rrm_share' , 50);
