...
function wpbeginner_numeric_posts_nav( $max_num_pages = 0 ) {
	...
	if ( ! in_array( 1, $links ) ) {
		$extra_class = ( 1 === $paged ) ? ' current' : '';
		printf(
			'<li><a class="page-numbers%s" href="%s">%s</a></li>' . "\n",
	...
	foreach ( (array) $links as $link ) {
		$extra_class = ( $paged === $link ) ? ' current' : '';
		printf(
			'<li><a class="page-numbers%s" href="%s">%s</a></li>' . "\n",
			esc_attr( $extra_class ),
	...
	if ( ! in_array( $max_num_pages, $links ) ) {
		...
		$extra_class = ( $paged === $max_num_pages ) ? ' current' : '';
		printf(
			'<li><a class="page-numbers%s" href="%s">%s</a></li>' . "\n",
			esc_attr( $extra_class ),
			esc_url( get_pagenum_link( $max_num_pages ) ),
			(int) $max_num_pages
		);
	}
...
function getLastPosts() {
	...
	while ( $wp_queryRecents->have_posts() ) :
		$wp_queryRecents->the_post();
				get_template_part(
...
function pending_posts( $new_status, $old_status, $post ) {
	...
	} elseif ( ( 'publish_to_trash' === $new_status || 'trash' === $new_status ) && 'galerias' === $post->post_type ) {
		$results = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM wphr_galerias_migration WHERE id_new = %d', $post->ID ) );
		foreach ( $results as $galeria ) {
			$wpdb->query( $wpdb->prepare( 'DELETE FROM wphr_galerias_migration WHERE id = %d', $galeria->id ) );
		}
